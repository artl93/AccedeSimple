...
def configure_opentelemetry():
    ...
    resource = Resource.create({
        "service.name": os.environ.get("OTEL_SERVICE_NAME", "localguide"),
        "service.version": "1.0.0"
    })

    # Setup tracing with resource
    otel_trace.set_tracer_provider(otel_sdk_trace.TracerProvider(resource=resource))
    ...
    otel_metrics.set_meter_provider(otel_sdk_metrics.MeterProvider(metric_readers=[metric_reader], resource=resource))

    # Setup logging with resource
    otel_logs.set_logger_provider(otel_sdk_logs.LoggerProvider(resource=resource))
